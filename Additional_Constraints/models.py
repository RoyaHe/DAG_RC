...
class KGReasoning(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma, 
                 ...
                 center_reg=None, drop=0., asso_pretrain=False, mono_pretrain=False):
    ...
    def forward(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, asso_pretrain, mono_pretrain):
        if self.geo == 'box':
            return self.forward_box(positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, asso_pretrain, mono_pretrain)
        elif self.geo == 'vec':
            return self.forward_vec(positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict)
        elif self.geo == 'beta':
            return self.forward_beta(positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, asso_pretrain, mono_pretrain)
        elif self.geo == "cone":
            return self.forward_cone(positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, asso_pretrain, mono_pretrain)
        
    def box_overlap(self, box1_embedding, box1_offset_embedding, box2_embedding, box2_offset_embedding):
        intersections_embedding = self.center_net(torch.stack([box1_embedding, box2_embedding]))
        intersections_offset_embedding = self.offset_net(torch.stack([box1_offset_embedding, box2_offset_embedding]))
        return intersections_embedding, intersections_offset_embedding
    
    def box_volumes(self, offset_embeddings):
        return F.softplus(offset_embeddings, beta=0.7).prod(1)

    def box_mono_loss_sub(self, base_embedding, base_offset_embedding, second_embedding, second_offset_embedding):
        intersection_embedding, intersection_offset_embedding = self.box_overlap(base_embedding, base_offset_embedding, second_embedding, second_offset_embedding)
        vol_intersection_box = self.box_volumes(intersection_offset_embedding)
        vol_base_box = self.box_volumes(base_offset_embedding)
        loss = torch.exp(torch.log(vol_base_box + 1e-10) - torch.log(vol_intersection_box + 1e-10))
        return loss

    def box_mono_loss_cal(self, embedding, offset_embedding, relation_embeddings, relation_offset_embeddings):
        
        mono_loss = 0
        if len(relation_embeddings) == 2:
            r_embedding_all, r_offset_embedding_all = self.BoxRelationCombinator(torch.stack(relation_embeddings), torch.stack(relation_offset_embeddings))
            base_embedding = embedding + r_embedding_all
            base_offset_embedding = offset_embedding + self.func(r_offset_embedding_all)

            for i in range(len(relation_embeddings)):
                second_embedding = embedding + relation_embeddings[i]
                second_offset_embedding = offset_embedding + relation_offset_embeddings[i]
                new_loss = self.box_mono_loss_sub(base_embedding, base_offset_embedding, second_embedding, second_offset_embedding)
                mono_loss += new_loss

        elif len(relation_embeddings) == 3:
            r_embedding_all, r_offset_embedding_all = self.BoxRelationCombinator(torch.stack(relation_embeddings), torch.stack(relation_offset_embeddings))
            base_embedding = embedding + r_embedding_all
            base_offset_embedding = offset_embedding + self.func(r_offset_embedding_all)

            for i in range(len(relation_embeddings)):
                tmp_relation_embeddings = torch.stack(relation_embeddings).clone()
                tmp_relation_offset_embeddings = torch.stack(relation_offset_embeddings).clone()

                tmp_relation_embeddings = torch.cat((tmp_relation_embeddings[:i], tmp_relation_embeddings[i+1:]), dim=0)
                tmp_relation_offset_embeddings = torch.cat((tmp_relation_offset_embeddings[:i], tmp_relation_offset_embeddings[i+1:]), dim=0)

                r_embedding_all, r_offset_embedding_all = self.BoxRelationCombinator(tmp_relation_embeddings, tmp_relation_offset_embeddings)
                second_embedding = embedding + r_embedding_all
                second_offset_embedding = offset_embedding + r_offset_embedding_all
                new_loss = self.box_mono_loss_sub(base_embedding, base_offset_embedding, second_embedding, second_offset_embedding)
                mono_loss += new_loss

                for j in range(tmp_relation_embeddings.shape[0]):
                    third_embedding = embedding + tmp_relation_embeddings[j,:,:]
                    third_offset_embedding = offset_embedding + tmp_relation_offset_embeddings[j,:,:]
                    new_loss = self.box_mono_loss_sub(second_embedding, second_offset_embedding, third_embedding, third_offset_embedding)
                    mono_loss += new_loss

        return mono_loss

    def embed_asso_box(self, queries, query_structure, idx):
        if query_structure[-1][-1] == 's':
            ## split
                ...
                for i in range(len(query_structure[-1])-1):
                    ...
                    embeddings.append(embedding + r_embedding)
                    offset_embeddings.append(offset_embedding + self.func(r_offset_embedding))

                embedding = self.center_net(torch.stack(embeddings))
                offset_embedding = self.offset_net(torch.stack(offset_embeddings))
    ...
    def embed_query_box(self, queries, query_structure, idx, mono_pretrain, mono_loss):
        '''
        ...
        if all_relation_flag:
            if query_structure[0] == 'e':
                ...
                embedding, offset_embedding, idx, tmp_mono_loss = self.embed_query_box(queries, query_structure[0], idx, mono_pretrain, mono_loss)
                mono_loss += tmp_mono_loss
            for i in range(len(query_structure[-1])):
            ...
            if query_structure[-1][-1] == 's':
                ...
                ## compute mono_loss
                if mono_pretrain:
                    mono_loss = self.box_mono_loss_cal(embedding, offset_embedding, relation_embeddings, relation_offset_embeddings)
                ...
                for i in range(len(query_structure)):
                    embedding, offset_embedding, idx, tmp_mono_loss = self.embed_query_box(queries, query_structure[i], idx, mono_pretrain, mono_loss)
                    mono_loss += tmp_mono_loss
        ...
        return embedding, offset_embedding, idx, mono_loss
    ...
    def beta_overlap(self, base_embedding, second_embedding):
        base_alpha_embedding, base_beta_embedding = torch.chunk(base_embedding, 2, dim=-1)
        second_alpha_embedding, second_beta_embedding = torch.chunk(second_embedding, 2, dim=-1)
        alpha_embedding, beta_embedding = self.center_net(torch.stack([base_alpha_embedding,second_alpha_embedding]), torch.stack([base_beta_embedding,second_beta_embedding]))
        intersection_embedding = torch.cat([alpha_embedding, beta_embedding], dim=-1)
        return intersection_embedding

    def beta_mono_loss_sub(self, base_embedding, second_embedding):
        intersection_embedding = self.beta_overlap(base_embedding, second_embedding)
        loss = torch.sum(torch.abs(intersection_embedding - base_embedding))
        return loss

    def beta_mono_loss_cal(self, alpha_embedding, beta_embedding, relation_embeddings):
        mono_loss = 0
        if len(relation_embeddings) == 2:
            r_embedding_all = self.BetaRelationCombinator(torch.stack(relation_embeddings))
            base_embedding = self.projection_net(torch.cat([alpha_embedding, beta_embedding], dim=-1), r_embedding_all)

            for i in range(len(relation_embeddings)):
                second_embedding = self.projection_net(torch.cat([alpha_embedding, beta_embedding], dim=-1), r_embedding_all)
                new_loss = self.beta_mono_loss_sub(base_embedding, second_embedding)
                mono_loss += new_loss
            ...
            for i in range(len(relation_embeddings)):
                ...
                r_embedding_all = self.BetaRelationCombinator(tmp_relation_embeddings)
                second_embedding = self.projection_net(torch.cat([alpha_embedding, beta_embedding], dim=-1), r_embedding_all)
                new_loss = self.beta_mono_loss_sub(base_embedding, second_embedding)
                mono_loss += new_loss

                for j in range(tmp_relation_embeddings.shape[0]):
                    third_embedding = self.projection_net(torch.cat([alpha_embedding, beta_embedding], dim=-1), tmp_relation_embeddings[j,:,:])
                    new_loss = self.beta_mono_loss_sub(second_embedding, third_embedding)
                    mono_loss += new_loss

        return mono_loss

    def embed_asso_beta(self, queries, query_structure, idx):
        if query_structure[-1][-1] == 's':
            ## split
                ...
                for i in range(len(query_structure[-1])-1):
                    r_embedding = torch.index_select(self.relation_embedding, dim=0, index=queries[:, idx])
                    tmp_embedding = self.projection_net(embedding, r_embedding)
                    tmp_alpha_embedding, tmp_beta_embedding = torch.chunk(tmp_embedding, 2, dim=-1)
                    alpha_embeddings.append(tmp_alpha_embedding)
                    beta_embeddings.append(tmp_beta_embedding)

                alpha_embedding, beta_embedding = self.center_net(torch.stack(alpha_embeddings), torch.stack(beta_embeddings))
            ...
            r_embedding = self.BetaRelationCombinator(torch.stack(relation_embeddings))

            tmp_embedding = self.projection_net(embedding, r_embedding)
            alpha_embedding, beta_embedding = torch.chunk(tmp_embedding, 2, dim=-1)
        
        return alpha_embedding, beta_embedding, idx

    def embed_query_beta(self, queries, query_structure, idx, mono_pretrain, mono_loss):
        '''
        ...
        if all_relation_flag:
            if query_structure[0] == 'e':
                ...
                alpha_embedding, beta_embedding, idx, tmp_mono_loss = self.embed_query_beta(queries, query_structure[0], idx, mono_pretrain, mono_loss)
                mono_loss += tmp_mono_loss
            ...
            if query_structure[-1][-1] == 's':
                ...
                ## compute mono_loss
                if mono_pretrain:
                    mono_loss = self.beta_mono_loss_cal(alpha_embedding, beta_embedding, relation_embeddings)
                ...
                beta_embedding_list = []
                for i in range(len(query_structure)):
                    alpha_embedding, beta_embedding, idx, tmp_mono_loss = self.embed_query_beta(queries, query_structure[i], idx, mono_pretrain, mono_loss)
                    mono_loss += tmp_mono_loss
        ...
        return alpha_embedding, beta_embedding, idx, mono_loss
    
    def cone_mono_loss_sub(self, second_axis_embedding, second_arg_embedding, third_axis_embedding, third_arg_embedding):
        intersection_axis_embedding, intersection_arg_embedding = self.cone_intersection(torch.stack([second_axis_embedding, third_axis_embedding]), torch.stack([second_arg_embedding, third_arg_embedding]))
        loss = torch.sum(torch.abs(intersection_axis_embedding - second_axis_embedding)) + torch.sum(torch.abs(intersection_arg_embedding - second_arg_embedding))
        return loss


    def cone_mono_loss_cal(self, axis_embedding, arg_embedding, axis_r_embeddings, arg_r_embeddings):
        mono_loss = 0
        if len(axis_r_embeddings) == 2:
            axis_r_embedding_all, arg_r_embedding_all = self.ConeRelationCombinator(torch.stack(axis_r_embeddings), torch.stack(arg_r_embeddings))
            base_axis_embedding, base_arg_embedding = self.cone_proj(axis_embedding, arg_embedding, axis_r_embedding_all, arg_r_embedding_all)

            for i in range(len(axis_r_embeddings)):
                second_axis_embedding, second_arg_embedding = self.cone_proj(base_axis_embedding, base_arg_embedding, axis_r_embeddings[i], arg_r_embeddings[i])
                new_loss = self.cone_mono_loss_sub(base_axis_embedding, base_arg_embedding, second_axis_embedding, second_arg_embedding)
                mono_loss += new_loss

        elif len(axis_r_embeddings) == 3:
            axis_r_embedding_all, arg_r_embedding_all = self.ConeRelationCombinator(torch.stack(axis_r_embeddings), torch.stack(arg_r_embeddings))
            base_axis_embedding, base_arg_embedding = self.cone_proj(axis_embedding, arg_embedding, axis_r_embedding_all, arg_r_embedding_all)

            for i in range(len(axis_r_embeddings)):
                tmp_axis_r_embeddings = torch.stack(axis_r_embeddings).clone()
                tmp_arg_r_embeddings = torch.stack(arg_r_embeddings).clone()

                tmp_axis_r_embeddings = torch.cat((tmp_axis_r_embeddings[:i], tmp_axis_r_embeddings[i+1:]), dim=0)
                tmp_arg_r_embeddings = torch.cat((tmp_arg_r_embeddings[:i], tmp_arg_r_embeddings[i+1:]), dim=0)

                axis_r_embedding_all, arg_r_embedding_all = self.ConeRelationCombinator(tmp_axis_r_embeddings, tmp_arg_r_embeddings)
                second_axis_embedding, second_arg_embedding = self.cone_proj(axis_embedding, arg_embedding, axis_r_embedding_all, arg_r_embedding_all)
                new_loss = self.cone_mono_loss_sub(base_axis_embedding, base_arg_embedding, second_axis_embedding, second_arg_embedding)
                mono_loss += new_loss

                for j in range(tmp_axis_r_embeddings.shape[0]):
                    third_axis_embedding, third_arg_embedding = self.cone_proj(axis_embedding, arg_embedding, tmp_axis_r_embeddings[j,:,:], tmp_arg_r_embeddings[j,:,:])
                    new_loss = self.cone_mono_loss_sub(second_axis_embedding, second_arg_embedding, third_axis_embedding, third_arg_embedding)
                    mono_loss += new_loss
        
        return mono_loss


    def embed_asso_cone(self, queries, query_structure, idx):
        if query_structure[-1][-1] == 's':
            ## split
                ...
                for i in range(len(query_structure[-1])-1):
                    ...
                    tmp_axis_embedding, tmp_arg_embedding = self.cone_proj(axis_embedding, arg_embedding, axis_r_embedding, arg_r_embedding)

                    axis_embeddings.append(tmp_axis_embedding)
                    arg_embeddings.append(tmp_arg_embedding)

                axis_embedding, arg_embedding = self.cone_intersection(torch.stack(axis_embeddings), torch.stack(arg_embeddings))
    ...
    def embed_query_cone(self, queries, query_structure, idx, mono_pretrain, mono_loss):
        all_relation_flag = True
        ...
        if all_relation_flag:
            if query_structure[0] == 'e':
                ...
                axis_embedding, arg_embedding, idx, tmp_mono_loss = self.embed_query_cone(queries, query_structure[0], idx, mono_pretrain, mono_loss)
                mono_loss += tmp_mono_loss
            ...
            if query_structure[-1][-1] == 's':
                ...
                ## compute mono_loss
                if mono_pretrain:
                    mono_loss = self.cone_mono_loss_cal(axis_embedding, arg_embedding, axis_r_embeddings, arg_r_embeddings)
                ...
                arg_embedding_list = []
                for i in range(len(query_structure)):
                    axis_embedding, arg_embedding, idx, tmp_mono_loss = self.embed_query_cone(queries, query_structure[i], idx, mono_pretrain, mono_loss)
                    mono_loss += tmp_mono_loss
        ...
        return axis_embedding, arg_embedding, idx, mono_loss
    ...
    def forward_beta(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, asso_pretrain, mono_pretrain):
        ...
        for query_structure in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure] and 'DNF' in self.query_name_dict[query_structure]:
                if not mono_pretrain:
                    alpha_embedding, beta_embedding, _, _ = \
                        self.embed_query_beta(self.transform_union_query(batch_queries_dict[query_structure], 
                                                                        query_structure), 
                                            self.transform_union_structure(query_structure), 
                                            0, False, 0)
                else:
                    alpha_embedding, beta_embedding, _, tmp_mono_loss = \
                        self.embed_query_beta(self.transform_union_query(batch_queries_dict[query_structure], 
                                                                        query_structure), 
                                            self.transform_union_structure(query_structure), 
                                            0, mono_pretrain, 0)
                ...
                alpha_embedding, beta_embedding, _, _= self.embed_query_beta(batch_queries_dict[query_structure], 
                                                                           query_structure, 
                                                                           0, False, 0)
                if mono_pretrain:
                    alpha_embedding, beta_embedding, _, tmp_mono_loss = \
                        self.embed_query_beta(batch_queries_dict[query_structure], query_structure, 0, mono_pretrain, 0)
                    mono_loss += torch.sum(tmp_mono_loss)

                if asso_pretrain:
                    ## compute 2rs-->2i and 3rs-->3i embeddings
                    asso_alpha_embedding, asso_beta_embedding, _ = self.embed_asso_beta(batch_queries_dict[query_structure], query_structure, 0)

                    ## compute the losses between 2rs and 2i, 3rs and 3i
                    tmp_asso_loss = torch.sum(torch.abs(asso_alpha_embedding - alpha_embedding) + torch.abs(asso_beta_embedding - beta_embedding))
                    asso_loss += tmp_asso_loss
        ...
        return positive_logit, negative_logit, subsampling_weight, all_idxs+all_union_idxs, asso_loss, mono_loss
    ...
    def forward_box(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, asso_pretrain, mono_pretrain):
        ...
        for query_structure in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure]:
                if not mono_pretrain:
                    center_embedding, offset_embedding, _, _= \
                        self.embed_query_box(self.transform_union_query(batch_queries_dict[query_structure], 
                                                                        query_structure), 
                                            self.transform_union_structure(query_structure), 
                                            0, False, 0)
                else:
                    center_embedding, offset_embedding, _, tmp_mono_loss= \
                        self.embed_query_box(self.transform_union_query(batch_queries_dict[query_structure], 
                                                                        query_structure), 
                                            self.transform_union_structure(query_structure), 
                                            0, mono_pretrain, 0)
                ...
                center_embedding, offset_embedding, _, _= self.embed_query_box(batch_queries_dict[query_structure],query_structure, 0, False, 0)

                if mono_pretrain:
                    center_embedding, offset_embedding, _, tmp_mono_loss= self.embed_query_box(batch_queries_dict[query_structure],query_structure, 0, mono_pretrain, 0)
                    mono_loss += torch.sum(tmp_mono_loss)

                if asso_pretrain:
                    ## compute 2rs-->2i and 3rs-->3i embeddings
                    asso_center_embedding, asso_offset_embedding, _ = self.embed_asso_box(batch_queries_dict[query_structure], query_structure, 0)

                    ## compute the losses between 2rs and 2i, 3rs and 3i
                    tmp_asso_loss = torch.sum(torch.abs(asso_center_embedding - center_embedding) + torch.abs(asso_offset_embedding - offset_embedding))
                    asso_loss += tmp_asso_loss
    ...
    def forward_cone(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, asso_pretrain, mono_pretrain):
        ...
        for query_structure in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure] and 'DNF' in self.query_name_dict[query_structure]:
                if not mono_pretrain:
                    axis_embedding, arg_embedding, _, _ = \
                        self.embed_query_cone(self.transform_union_query(batch_queries_dict[query_structure], query_structure), \
                                              self.transform_union_structure(query_structure), 0, False, 0)
                else:
                     axis_embedding, arg_embedding, _, tmp_mono_loss = \
                        self.embed_query_cone(self.transform_union_query(batch_queries_dict[query_structure], query_structure), \
                                              self.transform_union_structure(query_structure), 0, mono_pretrain, 0)
                     mono_loss += tmp_mono_loss
                all_union_idxs.extend(batch_idxs_dict[query_structure])
                ...
                axis_embedding, arg_embedding, _, _ = self.embed_query_cone(batch_queries_dict[query_structure], query_structure, 0, False, 0)

                if mono_pretrain:
                    axis_embedding, arg_embedding, _, tmp_mono_loss= self.embed_query_cone(batch_queries_dict[query_structure], query_structure, 0, mono_pretrain, 0)
                    mono_loss += torch.sum(tmp_mono_loss)

                if asso_pretrain:
                    ## compute 2rs-->2i and 3rs-->3i embeddings
                    asso_axis_embedding, asso_arg_embedding, _ = self.embed_asso_cone(batch_queries_dict[query_structure], query_structure, 0)

                    ## compute the losses between 2rs and 2i, 3rs and 3i
                    tmp_asso_loss = torch.sum(torch.abs(asso_axis_embedding - axis_embedding) + torch.abs(asso_arg_embedding - arg_embedding))
                    asso_loss += tmp_asso_loss
    ...
    @staticmethod
    def train_step(model, optimizer, train_iterator, train_asso_iterator, train_mono_iterator, args, step):
        model.train()
        optimizer.zero_grad()

        train_positive_sample, train_negative_sample, train_subsampling_weight, train_batch_queries, train_query_structures = next(train_iterator)
        if args.asso_pretrain:
            asso_positive_sample, asso_negative_sample, asso_subsampling_weight, asso_batch_queries, asso_query_structures = next(train_asso_iterator)
        if args.mono_pretrain:
            mono_positive_sample, mono_negative_sample, mono_subsampling_weight, mono_batch_queries, mono_query_structures = next(train_mono_iterator)

        ## Train losses
        train_batch_queries_dict = collections.defaultdict(list)
        train_batch_idxs_dict = collections.defaultdict(list)
        for i, query in enumerate(train_batch_queries): # group queries with same structure
            train_batch_queries_dict[train_query_structures[i]].append(query)
            train_batch_idxs_dict[train_query_structures[i]].append(i)
        for query_structure in train_batch_queries_dict:
            if args.cuda:
                train_batch_queries_dict[query_structure] = torch.LongTensor(train_batch_queries_dict[query_structure]).cuda()
            else:
                train_batch_queries_dict[query_structure] = torch.LongTensor(train_batch_queries_dict[query_structure])
        if args.cuda:
            train_positive_sample = train_positive_sample.cuda()
            train_negative_sample = train_negative_sample.cuda()
            train_subsampling_weight = train_subsampling_weight.cuda()


        train_positive_logit, train_negative_logit, train_subsampling_weight, _, _, _ = model(train_positive_sample, train_negative_sample, train_subsampling_weight, train_batch_queries_dict, train_batch_idxs_dict, False, False)

        train_negative_score = F.logsigmoid(-train_negative_logit).mean(dim=1)
        train_positive_score = F.logsigmoid(train_positive_logit).squeeze(dim=1)
        train_positive_sample_loss = - (train_subsampling_weight * train_positive_score).sum()
        train_negative_sample_loss = - (train_subsampling_weight * train_negative_score).sum()
        train_positive_sample_loss /= train_subsampling_weight.sum()
        train_negative_sample_loss /= train_subsampling_weight.sum()

        ## Asso losses
        if args.asso_pretrain:
            asso_batch_queries_dict = collections.defaultdict(list)
            asso_batch_idxs_dict = collections.defaultdict(list)
            for i, query in enumerate(asso_batch_queries): # group queries with same structure
                asso_batch_queries_dict[asso_query_structures[i]].append(query)
                asso_batch_idxs_dict[asso_query_structures[i]].append(i)
            for query_structure in asso_batch_queries_dict:
                if args.cuda:
                    asso_batch_queries_dict[query_structure] = torch.LongTensor(asso_batch_queries_dict[query_structure]).cuda()
                else:
                    asso_batch_queries_dict[query_structure] = torch.LongTensor(asso_batch_queries_dict[query_structure])
            if args.cuda:
                asso_positive_sample = asso_positive_sample.cuda()
                asso_negative_sample = asso_negative_sample.cuda()
                asso_subsampling_weight = asso_subsampling_weight.cuda()

            asso_positive_logit, asso_negative_logit, asso_subsampling_weight, _, asso_loss, _ = model(asso_positive_sample, asso_negative_sample, asso_subsampling_weight, asso_batch_queries_dict, asso_batch_idxs_dict, args.asso_pretrain, False)

            asso_negative_score = F.logsigmoid(-asso_negative_logit).mean(dim=1)
            asso_positive_score = F.logsigmoid(asso_positive_logit).squeeze(dim=1)
            asso_positive_sample_loss = - (asso_subsampling_weight * asso_positive_score).sum()
            asso_negative_sample_loss = - (asso_subsampling_weight * asso_negative_score).sum()
            asso_positive_sample_loss /= asso_subsampling_weight.sum()
            asso_negative_sample_loss /= asso_subsampling_weight.sum()
        
        if args.mono_pretrain:
            mono_batch_queries_dict = collections.defaultdict(list)
            mono_batch_idxs_dict = collections.defaultdict(list)
            for i, query in enumerate(mono_batch_queries): # group queries with same structure
                mono_batch_queries_dict[mono_query_structures[i]].append(query)
                mono_batch_idxs_dict[mono_query_structures[i]].append(i)
            for query_structure in mono_batch_queries_dict:
                if args.cuda:
                    mono_batch_queries_dict[query_structure] = torch.LongTensor(mono_batch_queries_dict[query_structure]).cuda()
                else:
                    mono_batch_queries_dict[query_structure] = torch.LongTensor(mono_batch_queries_dict[query_structure])
            if args.cuda:
                mono_positive_sample = mono_positive_sample.cuda()
                mono_negative_sample = mono_negative_sample.cuda()
                mono_subsampling_weight = mono_subsampling_weight.cuda()

            mono_positive_logit, mono_negative_logit, mono_subsampling_weight, _, _, mono_loss = model(mono_positive_sample, mono_negative_sample, mono_subsampling_weight, mono_batch_queries_dict, mono_batch_idxs_dict, False, args.mono_pretrain)

            mono_negative_score = F.logsigmoid(-mono_negative_logit).mean(dim=1)
            mono_positive_score = F.logsigmoid(mono_positive_logit).squeeze(dim=1)
            mono_positive_sample_loss = - (mono_subsampling_weight * mono_positive_score).sum()
            mono_negative_sample_loss = - (mono_subsampling_weight * mono_negative_score).sum()
            mono_positive_sample_loss /= mono_subsampling_weight.sum()
            mono_negative_sample_loss /= mono_subsampling_weight.sum()

        
        train_loss = (train_positive_sample_loss + train_negative_sample_loss)/2
        if args.asso_pretrain and (not args.mono_pretrain):
            asso_total_loss = (asso_positive_sample_loss + asso_negative_sample_loss)/2 + (args.asso_pretrain_weight * asso_loss)
            loss = 0.8 * train_loss + 0.2 * asso_total_loss
        elif (not args.asso_pretrain) and args.mono_pretrain:
            mono_total_loss = (mono_positive_sample_loss + mono_negative_sample_loss + args.mono_pretrain_weight * mono_loss)/3
            loss = 0.8 * train_loss + 0.2 * mono_total_loss
        elif args.asso_pretrain and args.mono_pretrain:
            asso_total_loss = (asso_positive_sample_loss + asso_negative_sample_loss + args.asso_pretrain_weight * asso_loss)/3
            mono_total_loss = (mono_positive_sample_loss + mono_negative_sample_loss + args.mono_pretrain_weight * mono_loss)/3
            loss = train_loss + 0.1 * asso_total_loss + 0.1 * mono_total_loss
        else:
            assert (args.asso_pretrain == False) and (args.mono_pretrain == False)
            loss = train_loss
        ...
        if args.asso_pretrain and (not args.mono_pretrain):
            log = {
                'train_positive_sample_loss': train_positive_sample_loss.item(),
                'train_negative_sample_loss': train_negative_sample_loss.item(),
                'asso_positive_sample_loss': asso_positive_sample_loss.item(),
                'asso_negative_sample_loss': asso_negative_sample_loss.item(),
                'asso_loss': args.asso_pretrain_weight * asso_loss.item(),
                'loss': loss.item()
            }
        elif (not args.asso_pretrain) and args.mono_pretrain:
            log = {
                'train_positive_sample_loss': train_positive_sample_loss.item(),
                'train_negative_sample_loss': train_negative_sample_loss.item(),
                'mono_positive_sample_loss': mono_positive_sample_loss.item(),
                'mono_negative_sample_loss': mono_negative_sample_loss.item(),
                'mono_loss': args.mono_pretrain_weight * mono_loss.item(),
                'loss': loss.item()
            }
        elif args.asso_pretrain and args.mono_pretrain:
            log = {
    ...
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False):
        ...
        with torch.set_grad_enabled(requires_grad):
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                _, negative_logit, _, idxs, _, _ = model(None, negative_sample, None, batch_queries_dict, batch_idxs_dict, False, False)
