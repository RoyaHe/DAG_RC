...
query_name_dict = {('e',('r',)): '1p', 
                    ...
                    (('e', ('r',)),('r','r','r','s')): '3s',
                    ((('e', ('r',)),('r','r','s')),('r',)): 'sp',
                    ((('e', ('r',)), ('e', ('r',)), ('u',)), ('r', 'r', 's')): 'us-DNF',
                    ((('e',('r',)),('e',('r',))), ('r', 'r','s')): 'is',
                    ((('e',('r','n')), ('e',('r',))), ('r','r','s')): 'ins',
                    ((('e',('r',)),('r',('r','n'),'s')), ('e',('r',))): 'pnsi',
                    ('e', ('r', 'r', 's')): '2rs',
                    ('e', ('r', 'r', 'r', 's')): '3rs'
                }
...
def parse_args(args=None):
    ...
    parser.add_argument('--tree_data_path', type=str, default=None, help="tree queries data path")
    parser.add_argument('--dag_data_path', type=str, default=None, help="dag queries data path")
    ...
    parser.add_argument('--save_checkpoint_steps', default=20000, type=int, help="save checkpoints every xx steps")
    ...
    parser.add_argument('--geo', default='vec', type=str, choices=['vec', 'box', 'beta', 'cone', 'cqd'], help='the reasoning model, vec for GQE, box for Query2box, beta for BetaE')
    ...
    parser.add_argument('--asso_pretrain', type=bool, default=False)
    parser.add_argument('--asso_pretrain_weight', default=1e-3, type=float)
    parser.add_argument('--mono_pretrain_weight', default=1e-4, type=float)
    parser.add_argument('--asso_path', type=str, default=None, help="pretrained asso queries data path")
    parser.add_argument('--mono_pretrain', type=bool, default=False)
...
def load_data(args, tasks):
    ...
    train_queries, train_answers, valid_queries, valid_hard_answers, valid_easy_answers = defaultdict(set), defaultdict(set), defaultdict(set), defaultdict(set), defaultdict(set)
    train_queries = pickle.load(open(os.path.join(args.tree_data_path, "train-queries.pkl"), 'rb'))
    train_answers = pickle.load(open(os.path.join(args.tree_data_path, "train-answers.pkl"), 'rb'))
    valid_queries = pickle.load(open(os.path.join(args.tree_data_path, "valid-queries.pkl"), 'rb'))
    valid_hard_answers = pickle.load(open(os.path.join(args.tree_data_path, "valid-hard-answers.pkl"), 'rb'))
    valid_easy_answers = pickle.load(open(os.path.join(args.tree_data_path, "valid-easy-answers.pkl"), 'rb'))
    test_queries = pickle.load(open(os.path.join(args.tree_data_path, "test-queries.pkl"), 'rb'))
    test_hard_answers = pickle.load(open(os.path.join(args.tree_data_path, "test-hard-answers.pkl"), 'rb'))
    test_easy_answers = pickle.load(open(os.path.join(args.tree_data_path, "test-easy-answers.pkl"), 'rb'))
    
    ## load dag queries
    #dag_train_queries, dag_train_answers, dag_valid_queries, dag_valid_hard_answers, dag_valid_easy_answers = defaultdict(set), defaultdict(set), defaultdict(set), defaultdict(set), defaultdict(set)
    dag_train_queries = pickle.load(open(os.path.join(args.dag_data_path, "train-queries.pkl"), 'rb'))
    dag_train_answers = pickle.load(open(os.path.join(args.dag_data_path, "train-answers.pkl"), 'rb'))
    dag_valid_queries = pickle.load(open(os.path.join(args.dag_data_path, "valid-queries.pkl"), 'rb'))
    dag_valid_hard_answers = pickle.load(open(os.path.join(args.dag_data_path, "valid-hard-answers.pkl"), 'rb'))
    dag_valid_easy_answers = pickle.load(open(os.path.join(args.dag_data_path, "valid-easy-answers.pkl"), 'rb'))
    dag_test_queries = pickle.load(open(os.path.join(args.dag_data_path, "test-queries.pkl"), 'rb'))
    dag_test_hard_answers = pickle.load(open(os.path.join(args.dag_data_path, "test-hard-answers.pkl"), 'rb'))
    dag_test_easy_answers = pickle.load(open(os.path.join(args.dag_data_path, "test-easy-answers.pkl"), 'rb'))

    ## combine queries
    train_queries.update(dag_train_queries)
    train_answers.update(dag_train_answers)
    valid_queries.update(dag_valid_queries)
    valid_hard_answers.update(dag_valid_hard_answers)
    valid_easy_answers.update(dag_valid_easy_answers)
    test_queries.update(dag_test_queries)
    test_hard_answers.update(dag_test_hard_answers)
    test_easy_answers.update(dag_test_easy_answers)
    ...
    if args.asso_path is not None:
        ## load 2rs and 3rs queries
        logging.info("loading associativity data")
        asso_2rs_queries, asso_3rs_queries, asso_2rs_answers, asso_3rs_answers = defaultdict(set), defaultdict(set), defaultdict(set), defaultdict(set)
        asso_2rs_queries = pickle.load(open(os.path.join(args.asso_path, "train-6-queries.pkl"), 'rb'))
        asso_3rs_queries = pickle.load(open(os.path.join(args.asso_path, "train-7-queries.pkl"), 'rb'))
        asso_2rs_answers = pickle.load(open(os.path.join(args.asso_path, "train-6-fn-answers.pkl"), 'rb'))
        asso_3rs_answers = pickle.load(open(os.path.join(args.asso_path, "train-7-fn-answers.pkl"), 'rb'))
        train_queries[('e', ('r', 'r', 's'))] = asso_2rs_queries[('e', ('r', 'r', 's'))]
        train_queries[('e', ('r', 'r', 'r', 's'))] = asso_3rs_queries[('e', ('r', 'r', 'r','s'))]
        for query in asso_2rs_answers.keys():
            train_answers[query] = asso_2rs_answers[query]
        for query in asso_3rs_answers.keys():
            train_answers[query] = asso_3rs_answers[query]
...
def main(args):
    ...
    if args.do_train:
        args.save_path = os.path.join(prefix, args.dag_data_path.split('/')[-2], args.tasks, args.geo)
    else:
        args.save_path = os.path.join(args.dag_data_path.split('/')[-2], args.dag_data_path.split('/')[-1], args.tasks, args.geo)
    if args.geo in ['box']:
        ...
        tmp_str = "g-{}-drop-{}".format(args.gamma, args.drop)
    elif args.geo == 'cqd':
        tmp_str = "g-cqd"
    ...
    with open('%s/stats.txt'%args.dag_data_path) as f:
        entrel = f.readlines()
    ...
    logging.info('Tree Data Path: %s' % args.tree_data_path)
    logging.info('DAG Data Path: %s' % args.dag_data_path)
    ...
    if args.do_train:
        ...
        train_asso_queries = defaultdict(set)
        train_mono_queries = defaultdict(set)
        path_list = ['1p', '2p', '3p']
        asso_list = ['2rs', '3rs']
        mono_list = ['2s', '3s', 'sp', 'is', 'us', 'ins']
        for query_structure in train_queries:
            if query_name_dict[query_structure] in path_list:
                train_path_queries[query_structure] = train_queries[query_structure]
            else:
                if query_name_dict[query_structure] in asso_list:
                    train_asso_queries[query_structure] = train_queries[query_structure]
                    train_other_queries[query_structure] = train_queries[query_structure]
                else:
                    if query_name_dict[query_structure] in mono_list and args.mono_pretrain:
                        train_mono_queries[query_structure] = train_queries[query_structure]
                        train_other_queries[query_structure] = train_queries[query_structure]
                    else:
                        train_other_queries[query_structure] = train_queries[query_structure]
        ...
        if len(train_asso_queries) > 0:
            train_asso_queries = flatten_query(train_asso_queries)
            train_asso_iterator = SingledirectionalOneShotIterator(DataLoader(
                                        TrainDataset(train_asso_queries, nentity, nrelation, args.negative_sample_size, train_answers),
                                        batch_size=args.batch_size,
                                        shuffle=True,
                                        num_workers=args.cpu_num,
                                        collate_fn=TrainDataset.collate_fn
            ...
            train_asso_iterator = None
        if len(train_mono_queries) > 0:
            train_mono_queries = flatten_query(train_mono_queries)
            train_mono_iterator = SingledirectionalOneShotIterator(DataLoader(
                                        TrainDataset(train_mono_queries, nentity, nrelation, args.negative_sample_size, train_answers),
                                        batch_size=args.batch_size,
                                        shuffle=True,
                                        num_workers=args.cpu_num,
                                        collate_fn=TrainDataset.collate_fn
    ...
    if args.geo == 'cqd':
        ...
        model = KGReasoning(
            ...
            drop = args.drop,
            asso_pretrain=args.asso_pretrain,
            mono_pretrain = args.mono_pretrain
        )
    ...
    if args.geo == 'box':
        ...
        logging.info('cqd-k: %d' % args.cqd_k)
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if train_path_iterator is not None:
                log = model.train_step(model, optimizer, train_path_iterator, train_asso_iterator, train_mono_iterator, args, step)
                for metric in log:
                    writer.add_scalar('path_'+metric, log[metric], step)
            if train_other_iterator is not None:
                a = next(train_other_iterator)
                log = model.train_step(model, optimizer, train_other_iterator, train_asso_iterator, train_mono_iterator, args, step)
                for metric in log:
